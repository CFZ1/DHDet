...
class applyClahe(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        Returns:
            dict: ColorJitter results, 'img' key is updated in result dict.
        """
        img = results['img'] #cv2.imwrite('/media/zcf/extra/zcf/code/231108_FoundationModels/mmyolo_forv8/mmyolo/1.jpg', img)
        # 读取图像
...
class adjustBrightness(BaseTransform):
    def __init__(self,
                 brightness =1.0,
                 mode='mmcv', mmcv_backend = 'pillow'):
        '''
        mode: mmcv, self
        mmcv_backend: pillow, cv2
        '''
        self.brightness = brightness
        self.mode = mode
        self.mmcv_backend = mmcv_backend
        
        logger = MMLogger.get_current_instance()
        logger.info(f'adjustBrightness {mode}===>: brightness={self.brightness}, mmcv_backend={self.mmcv_backend}')
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if self.brightness !=1.0:
            if self.mode == 'mmcv':
                img = mmcv.adjust_brightness(
                    img, self.brightness, backend=self.mmcv_backend)
            elif self.mode == 'self':
                dtype = img.dtype
                maxval = MAX_VALUES_BY_DTYPE.get(dtype, 1.0)
                img = self.brightness * img + (1-self.brightness)* np.zeros_like(img)
...
def motion_blur(img, ksize, angle):
    ...
    rot_mat = cv2.getRotationMatrix2D((ksize / 2 - 0.5, ksize / 2 - 0.5), angle, 1.0)
    kernel = cv2.warpAffine(kernel, rot_mat, (ksize, ksize))
    # 卷积
    return cv2.filter2D(img, -1, kernel)    
@TRANSFORMS.register_module()
class addMotionBlur(BaseTransform):
    def __init__(self,
                 ksize =9, angle=(0,360)
                 ):
        '''
        ...
        logger = MMLogger.get_current_instance()
        logger.info(f'addMotionBlur===>: ksize={self.ksize}, angle={self.angle}')
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        """
        img = results['img'] 
        blurred_img = motion_blur(img,self.ksize,self.angle)
        results['img'] = blurred_img
        return results
    
@TRANSFORMS.register_module()
class addGaussianBlur(BaseTransform):
    def __init__(self,
                 kernelS =(3, 3), sigma=0.5
                 ):
        '''
        '''
        ...
        logger = MMLogger.get_current_instance()
        logger.info(f'addGaussianBlur===>: kernelS={self.kernelS}, sigma={self.sigma}')
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        """
        img = results['img'] 
        blurred_img = cv2.GaussianBlur(img, self.kernelS, sigmaX=self.sigma, sigmaY=self.sigma)
        # import torchvision
        # blurred_img =  torchvision.transforms.functional.gaussian_blur(img, self.kernelS[0], sigma=(self.sigma,self.sigma))
        results['img'] = blurred_img
        return results
...
#         # img = cv2.imread(image_path)
#         # 应用形态学闭运算增强细小划痕
#         img = cv2.morphologyEx(img, cv2.MORPH_CLOSE, self.kernel, iterations=self.iterations)
#         # 转换到LAB色彩空间
#         lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
#         l, a, b = cv2.split(lab)
#         # 应用CLAHE到L通道
