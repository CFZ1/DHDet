...
from mmdet.models import DINOHead,DeformableDETRHead
...
from mmdet.models.layers import CdnQueryGenerator,DinoTransformerDecoder
...
class BboxDINOHead(DINOHead): 
    ...
    def __init__(self,
        ...
        if share_decoder_withLineHead['shared'] or decoder is None:
            ...
            self.decoder = DinoTransformerDecoder(**decoder) #3.---------DINO._init_layers
        #2.---------DeformableDETR
        ...
        if dn_cfg is not None:
            ...
            dn_cfg['num_classes'] = self.num_classes # 1.zhou self.bbox_head-->self.
    ...
    def init_weights(self) -> None:
        ...
        if not self.share_decoder_withLineHead['shared']:
            for coder in self.encoder, self.decoder: #!TODO self.encoder is None
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: Union[list,dict]):
        ...
        losses = super().loss(**head_inputs_dict, batch_data_samples=batch_data_samples)
        return losses
    ...
    def forward_transformer(self,
        ...
        tmp_dec_in, head_inputs_dict = self.pre_decoder(**encoder_outputs_dict, batch_data_samples=batch_data_samples) #DINO
    ...
    def pre_decoder(
        ...
        bs, _, c = memory.shape
        cls_out_features = self.cls_branches[ # 1.zhou self.bbox_head-->self.
            self.decoder.num_layers].out_features

        output_memory, output_proposals = self.gen_encoder_output_proposals(
            memory, memory_mask, spatial_shapes)
        enc_outputs_class = self.cls_branches[ # 1.zhou self.bbox_head-->self.
            self.decoder.num_layers](
                output_memory)
        enc_outputs_coord_unact = self.reg_branches[ # 1.zhou self.bbox_head-->self.
    ...
    def forward_decoder(self,
        ...
        inter_states, references = self.decoder(
            ...
            reg_branches=self.reg_branches, # 1.zhou self.bbox_head-->self.
            **kwargs)
