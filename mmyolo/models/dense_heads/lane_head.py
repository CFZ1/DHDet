...
# ------------------------------------------------------------------------
# copy from TopoMLP-master/projects/topomlp/models/heads
import torch
...
from .lane_head_utils import PETRTransformer
...
class LaneHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        assert self.num_reg_dim % pts_dim == 0 #------------zhou
        self.bev_range = kwargs['bev_range']
        self.num_control_points = int(self.num_reg_dim / pts_dim)
        ...
        self.position_dim = pts_dim * self.depth_num
        ...
        self.with_position, self.with_multiview = False, False #TODO!
        ...
        self.positional_encoding = build_positional_encoding(
            positional_encoding)
        # if transformer.type =='PETRTransformer':
        #     transformer.pop('type')
        #     self.transformer = PETRTransformer(**transformer)
    ...
    def _init_layers(self):
        ...
        self.reference_points_lane = nn.Embedding(self.num_lane, pts_dim)
        self.query_embedding_lane = nn.Sequential(
            nn.Linear(self.embed_dims * pts_dim // 2, self.embed_dims),
            nn.ReLU(),
    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        ...
        # coords = torch.cat((coords, torch.ones_like(coords[..., :1])), -1)  # W, H, 2+1
        ...
        # coords_mask = (coords > 1.0) | (coords < 0.0)
        # coords_mask = coords_mask.flatten(-2).sum(-1) > (D * 0.5)
        # coords_mask = masks | coords_mask.permute(0, 1, 3, 2)
        # coords = coords.permute(0, 1, 4, 5, 3, 2).contiguous().view(B * N, -1, H, W)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        # if (version is None or version < 2) and self.__class__ is PETRHeadlane_H: #self.__class__=<class 'mmyolo.models.dense_heads.lane_head.LaneHead'>
        #     convert_dict = {
    ...
    def forward(self, mlvl_feats, batch_data_samples):
        ...
        x = self.input_proj(x)
        # x = x.view(batch_size, *x.shape[-3:])
        # interpolate masks to have the same spatial shape with x
        masks = F.interpolate(
            masks[None], size=x.shape[-2:]).to(torch.bool).squeeze(0) #bs,h,w-->bs,1,h,w-->bs,h,w
        if self.with_position:
            ...
            if self.with_multiview:
                ...
                xy_embed = self.positional_encoding(masks)
            ...
            if self.with_multiview:
                ...
                # for i in range(num_cams):
                pos_embed = self.positional_encoding(masks)
                pos_embed = self.adapt_pos3d(pos_embed) #zhou
        ...
        query_embeds = self.query_embedding_lane(pos2posemb2d(reference_points_lane))
        # [batch_size, num_query=1800, self.num_control_points, 2]
        ...
        # x=[bs,dim,h,w], masks=[bs,h,w], query_embeds=[num_lanes=1800,dim], pos_embed(x)=[bs,dim,h,w],self_attn_mask=[num_lanes=1800,num_lanes=1800]
        # not use self.lane_reg_branches
        
        outs_dec, _ = self.transformer(x, masks, query_embeds, pos_embed, self.lane_reg_branches,
                                        decoder_self_attn_mask=[self_attn_mask, None], ) #outs_dec=[num_layers=6, bs, num_query=1800, dim]
        # outs_dec = torch.randn([6,batch_size,reference_points_lane.shape[1],256],device=reference_points_lane.device)
        # print('error error error error error error')
        
        if torch.__version__ >= '1.8.0':
            outs_dec = torch.nan_to_num(outs_dec)

        lane_queries = outs_dec #[num_layers=6, bs, num_query=1800, dim]
        outputs_lane_preds = []
        outputs_lane_clses = []
        ...
        for img_meta in batch_data_samples:
            ...
            factor = outs_dec.new_tensor([img_w, img_h]).unsqueeze(0)
            factors.append(factor)
        factors = torch.cat(factors, 0) 
        expanded_factors = factors.unsqueeze(1).unsqueeze(2).repeat(1, 1,self.num_control_points,1)  #[bs,1,4,2]
        expanded_factors = expanded_factors.flatten(2) #[bs,1,4*2]
        
        for lvl in range(outs_dec.shape[0]):
            reference = inverse_sigmoid(reference_points_lane.clone()) #TODO! why
            reference = reference.view(batch_size, self.num_lane, self.num_reg_dim)
            tmp = self.lane_reg_branches[lvl](lane_queries[lvl]) #[bs, self.num_lane, self.num_reg_dim=4*2]
            outputs_lanecls = self.lane_cls_branches[lvl](lane_queries[lvl]) #[bs, self.num_lane, num_cls=1]
            ...
            outputs_coord = tmp 
            outputs_coord = outputs_coord*expanded_factors #w,h,w,h
            outputs_lane_preds.append(outputs_coord)
            outputs_lane_clses.append(outputs_lanecls)

        all_lane_preds = torch.stack(outputs_lane_preds)  # torch.Size([num_layers=6, bs, num_query=1800, num_reg_dim=4*2])
        all_lane_clses = torch.stack(outputs_lane_clses)  # torch.Size([num_layers=6, bs, num_query=1800, num_cls=1])
        
        # for p in range(self.num_reg_dim // pts_dim):
        #     all_lane_preds[..., pts_dim * p] = all_lane_preds[..., pts_dim * p] #---------- 
        #     all_lane_preds[..., pts_dim * p + 1] = all_lane_preds[..., pts_dim * p + 1]   #---------- 
        ...
        all_lane_preds_one2one = all_lane_preds[:, :, 0: self.num_lanes_one2one, :] #[num_layers=6, bs, num_query=300, num_reg_dim=4*2]
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: Union[list,dict]) -> dict:
        if len(x) > 1:
            x = x[-2]
        all_cls_scores_list, all_bbox_preds_list, _,\
        all_cls_scores_one2many_list, all_bbox_preds_one2many_list, _ = self(x,batch_data_samples)
        gt_bboxes_ignore=None #TODO!
        ...
        for data_sample in batch_data_samples:
            img_metas.append(data_sample.metainfo)
            gt_bboxes_list.append(data_sample.gt_line_instances.line_points.flatten(1)) #gt_line_instances
            gt_labels_list.append(data_sample.gt_line_instances.line_labels) #gt_line_instances
        ...
        loss_dict['line_loss_cls'] = losses_cls[-1]
        loss_dict['line_loss_cls_H'] = losses_cls_one2many[-1] * self.lambda_one2many
        loss_dict['line_loss_bbox'] = losses_bbox[-1]
        loss_dict['line_loss_bbox_H'] = losses_bbox_one2many[-1] * self.lambda_one2many
        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i in zip(losses_cls[:-1],
                                                       losses_bbox[:-1],
                                                       losses_iou[:-1]):
            loss_dict[f'd{num_dec_layer}.line_loss_cls'] = loss_cls_i
            loss_dict[f'd{num_dec_layer}.line_loss_cls_H'] = losses_cls_one2many[num_dec_layer] * self.lambda_one2many
            loss_dict[f'd{num_dec_layer}.line_loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.line_loss_bbox_H'] = losses_bbox_one2many[num_dec_layer] * self.lambda_one2many
            num_dec_layer += 1
        return loss_dict
    ...
    def _get_target_single(self,
        ...
        if self.sampler:
            pred_instances.priors = gt_labels.new_zeros(cls_score.shape[0]) #TODO! for PseudoSampler
    ...
    #                img_metas,
    #                rescale=False):
    def predict(self, x: Tuple[Tensor], batch_data_samples: Union[list,dict], rescale: bool = True) -> InstanceList:
        ...
        index_lc = torch.sigmoid(all_cls_scores_list[-1]) > 0.0
        index_lc = index_lc.squeeze(-1)
        all_cls_scores_list = all_cls_scores_list[-1]
        all_bbox_preds_list = all_bbox_preds_list[-1]
        # all_cls_scores_list = all_cls_scores_list[:, :, index_lc, :]
        # all_bbox_preds_list= all_bbox_preds_list[:, :, index_lc, :]
        ...
        for img_id in range(len(img_metas)):
            cls_score = all_cls_scores_list[img_id, index_lc[img_id], :]
            bbox_pred = all_bbox_preds_list[img_id, index_lc[img_id], :]
            # cls_score = cls_scores[img_id]
            # bbox_pred = bbox_preds[img_id]
            img_shape = img_metas[img_id]['img_shape'] #h,w
            scale_factor = img_metas[img_id]['scale_factor'] #w,h
            bbox_pred[:, 0::2].clamp_(min=0, max=img_shape[1]) #w
            bbox_pred[:, 1::2].clamp_(min=0, max=img_shape[0]) #h
            if rescale:
                bbox_pred /= bbox_pred.new_tensor(
                    scale_factor).repeat((1, self.num_control_points)) #w,h,w,h...
                # batch_data_samples[img_id].gt_line_instances.line_points /= bbox_pred.new_tensor( #---add zhou
                #     scale_factor).repeat((1, n_points)).reshape(-1,n_points,pts_dim) #w,h,w,h...
            det_bboxes, scores, det_labels = self._get_bboxes_single(cls_score, bbox_pred,
                                                img_shape, scale_factor)
            results = InstanceData()
            results.line_points = det_bboxes #[num_queries,num_points*self.pts_dim] .reshape(det_bboxes.shape[0], -1, pts_dim)
    ...
    def _get_bboxes_single(self,
        ...
        det_bboxes = bbox_pred ##[num_query=300, num_reg_dim=4*2]
    ...
    def control_points_to_lane_points(self, lanes):
        lanes = lanes.reshape(-1, lanes.shape[-1] // pts_dim, pts_dim)
