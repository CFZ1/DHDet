...
class PETRTransformer(BaseModule):
    ...
    def forward(self, x, mask, query_embed, pos_embed, reg_branch=None, decoder_self_attn_mask=None, target=None):
        ...
        if self.cross:
            ...
            pos_embed = pos_embed.view(bs, c, -1).permute(2, 0, 1) # [bs, c, h, w] -> [h*w, bs, c]
        ...
        if target is None:
            target = torch.zeros_like(query_embed) #[num_query, bs, dim]
        ...
        # out_dec: [num_layers=6, num_query=1800, bs, dim]
        out_dec = self.decoder(
            query=target, #[num_query, bs, dim]
            key=memory, #[h*w, bs, c]
            value=memory, #[h*w, bs, c]
            key_pos=pos_embed, #[h*w, bs, c]
        ...
        memory = memory.reshape(h, w, bs, c).permute(2, 3, 0, 1) #[bs,c,h,w]
        return  out_dec, memory #[num_layers=6, bs, num_query=1800, dim], #[bs,c,h,w]
...
class PETRTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self, query, *args, **kwargs):
        ...
        for layer in self.layers:
            query = layer(query, *args, **kwargs) #[num_query, bs, dim]
            ...
            # print(query.size())
        return torch.stack(intermediate) #[num_layers=6,num_query=1800, bs, dim]
