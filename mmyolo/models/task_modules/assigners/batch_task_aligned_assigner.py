...
# from mmengine.logging import MMLogger #------------zcf detect TP 240229
...
class BatchTaskAlignedAssigner(nn.Module):
    ...
    def forward(
        ...
        #------------zcf detect TP 240229
        # num_TP = torch.sum((gt_labels == 0) & (pad_bbox_flag == 1))
        # logger: MMLogger = MMLogger.get_current_instance()
        # if num_TP >3:
        #     logger.info('每个gt的pos的topk个数 %s', pos_mask.sum(-1)) #[bs,num_gt,num_feas]
        #------------zcf detect TP 240229
        (assigned_gt_idxs, fg_mask_pre_prior,
         pos_mask) = select_highest_overlaps(pos_mask, overlaps, num_gt)
        #------------zcf detect TP 240229
        # if num_TP >3:
        #     logger.info('每个gt的pos的topk and highest_overlaps个数 %s', pos_mask.sum(-1))
        #------------zcf detect TP 240229
        # assigned target
        ...
        assigned_result['assigned_labels'] = assigned_labels #if pos =gt labels
        assigned_result['assigned_bboxes'] = assigned_bboxes #if pos =gt bboxes
    ...
    def get_pos_mask(self, pred_bboxes: Tensor, pred_scores: Tensor,
        ...
        # logger: MMLogger = MMLogger.get_current_instance()
        # if num_TP >3:
        #     logger.info('每个gt的pos个数 %s', is_in_gts.sum(-1)) #[bs,num_gt,num_feas]
        #     logger.info('gt_labels %s',gt_labels.squeeze(-1)) #[bs,num_gt,1]
        #     logger.info('pad_bbox_flag %s',pad_bbox_flag.squeeze(-1)) #[bs,num_gt,1]
        #------------zcf detect TP 240229
