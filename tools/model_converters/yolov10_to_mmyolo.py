...
convert_dict_s = {
    ...
    'model.10': 'backbone.stage4.3', #--------------not same as yolov8
    # reduce_layers 3× and upsample_layers 2×
    # neck
    'model.13': 'neck.top_down_layers.0', #12+1---not same as yolov8
    'model.16': 'neck.top_down_layers.1', #15+1---
    'model.17': 'neck.downsample_layers.0', #16+1---
    'model.19': 'neck.bottom_up_layers.0', #TODO, 18+1, C2fCIB !=CSPLayerWithTwoConv
    'model.20': 'neck.downsample_layers.1', #19+1---
    'model.22': 'neck.bottom_up_layers.1', #21+1---

    # Detector
    'model.23.one2one_cv2': 'bbox_head.head_module.one2many_cls_preds',  #TODO, ---not same as yolov8
    'model.23.cv2': 'bbox_head.head_module.one2many_reg_preds',  #---not same as yolov8
}
...
def convert(src, dst):
    ...
    for key, weight in blobs.items(): #act--->activate
        num, module = key.split('.')[1:3]
        prefix = f'model.{num}'
        new_key = key.replace(prefix, convert_dict[prefix])
        if 'backbone.stage3.0' or 'backbone.stage4.0' or 'backbone.stage4.1' or  'backbone.stage4.2' or  'backbone.stage4.3' or 'neck.downsample_layers.1' or 'neck.bottom_up_layers' in new_key: #---not same as yolov8
            continue
        if '.m.' in new_key:
