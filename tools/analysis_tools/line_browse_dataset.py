...
def main():
    ...
    Selected_image_ids = [0,105,292,359] #!TODO-----zhou
    Selected_image_ids = [105,292,359]
    for i in Selected_image_ids:
        item = dataset[i]    
    # for i, item in zip(range(display_number), dataset):
    #     #-----zhou
    #     if i not in Selected_image_ids:
    #     #----zhou
        image_i = []
        ...
        for k, datasample in enumerate(result_i):
            ...
            #----------zhou
            filename_new = osp.splitext(osp.basename(datasample.img_path))[0]+f'_{k}.jpg'
            out_file_new = osp.join(args.out_dir, filename_new)
            #----------zhou
